(() => {
  ...
  function buildYamadaAnchorOverlayLines(slotId, stage, draftState, encounterId) {
    ...
    if (stage === "encounter") {
      ...
      if (slotId === "3.3") {
        lines.push(`尸体摆在眼前时，${target}只要多看你两秒，就会发现你先盯的是血迹和伤口。`);
      }
      if (slotId === "4.4") {
        lines.push(`投票前后，${target}只要再抬高一点声音，旁人的视线就会被一起带偏。`);
    ...
    if (stage === "outcome") {
      ...
      if (slotId === "3.3") lines.push("这下你再装外行，只会显得更可疑。");
      if (slotId === "4.4") lines.push("票面一亮，你之前故意留白的部分也会被人顺手补全。");
      if (slotId === "5.4") lines.push("门口这一步会逼你承认，谁被你护住，谁就会成为你的账。");
    }
    ...
    if (stage === "ripple") {
      ...
      if (slotId === "4.4") {
        lines.push("这票之后，谁跟你站得近一点，都会被一起记进同一边。");
      }
      if (slotId === "5.1") {
  ...
  function buildDeboraAnchorOverlayLines(slotId, stage, draftState, encounterId) {
    ...
    if (stage === "opening") {
      if (slotId === "2.4") {
        lines.push("第二次聚集前，你最担心的不是该说什么，而是哪句话一旦说得太准，就会让人意识到你并不像表面那样只是个会打圆场的大人。你得把真正有用的判断拆散，掺进玩笑和迟疑里，一点点放出去。");
      }
      if (slotId === "3.3") {
        lines.push("尸体真正出现时，最先醒来的不是情绪，而是职业化的视线。哪里该看，哪里别碰，什么沉默说明现场被动过手脚，你几乎一眼就能分出来；也正因为太快，你才更要把这份熟练压回那层无害的大人外壳里。");
      }
      if (slotId === "4.4") {
        lines.push("投票室的秩序感会让你格外不舒服，因为它太像另一类现场整理。区别只在于，这一次摆上桌面的不只是尸体和证据，还有每个人早就不干净的动机。");
        if (karlExposed) lines.push("卡尔一旦露出裂口，你就知道另一种风险也跟着逼近。你比很多人都清楚，这类人最恨的往往不是被反对，而是被人完整记住自己失控时的样子。");
      }
      if (slotId === "5.1") {
        lines.push("异变发生时，你会比周围人更快地意识到自己那层“没什么用的大人”外壳正在整片剥落。生存反射、旧案记忆和那种靠冷静活下来的身体经验会一起顶上来，逼你承认你并不只是会安慰两句的人。");
      }
      if (slotId === "5.4") {
        lines.push("到了白门前，你已经很难再轻松地把自己藏在玩笑和装糊涂后面。结局逼你回答的，是一个比这栋楼更早就存在的问题：你到底想把自己活着带出去，还是想把那层“至少看起来像个好人”的样子也一起保住。");
    ...
    if (stage === "encounter") {
      if (slotId === "2.4") {
        lines.push(`第二次聚集前后和${target}说话时，你得先让${target}愿意低估你。只有这样，后面那些真正要命的观察才有机会悄悄留下。`);
      }
      if (slotId === "3.3") {
        lines.push(`尸体旁边再面对${target}，你最小心的反而是自己的眼神。你知道“太会看现场的人”有多容易露馅，所以哪怕已经看明白了一半，也得先把那份明白咽回圆场和玩笑后面。`);
      }
      if (slotId === "4.4") {
        lines.push(`到了投票前后，你和${target}之间的很多话都不再只是试探，而像提前写给之后口供的草稿。你每多问一句，别人就越可能回头追问你为什么偏偏在这种时候问得这么准。`);
      }
      if (slotId === "5.1") {
      ...
      if (slotId === "5.4") {
        lines.push(`白门前的${target}会让你格外清楚，“谁该先走”不是道理题。你和${target}之间此刻交换的不是礼貌和情报，而是最后关头谁愿意替谁担一点脏。`);
      }
    }

    if (stage === "outcome") {
      if (slotId === "2.4") {
        lines.push("把判断带回人群之后，你最先得到的不是轻松，而是新的风险。别人也许还说不清自己察觉到了什么，可那种“她怎么会这么熟”的疑心一旦种下，之后就很难再挖掉。");
      }
      if (slotId === "3.3") {
        lines.push("尸体出现后的后果也格外直接。你比大多数人更清楚，一次处理不当的现场会在后来留下多少后患，而这种清楚让你连呼吸都像背着旧案。");
      }
      if (slotId === "4.4") {
        lines.push("投票阶段最讨厌的地方在于，你已经不能只当那个帮人打圆场的大人了。哪怕嘴上仍旧轻巧，实际上你每一句话都可能在替某个人垫起或挖空最后的立足点。");
      }
      if (slotId === "5.1") {
        lines.push("异变把你身上那份“会处理残局的人”整个唤醒了。它并不让你骄傲，只会让你更累，因为你太清楚自己当初就是靠这种冷静活下来的。");
      }
      if (slotId === "5.4") {
        lines.push("越靠近白门，你越难继续把求生和赎罪分开。很多动作看上去像为了活下去，可你心里明白，其中有一部分不过是在偿还那些后来再也补不回来的决定。");
        if (emilyTrust >= 12) lines.push("艾米莉相关的结果尤其会压得你更沉。只要你真的替她伸过手，之后每一次想抽身，都会像在重复一次你最讨厌的见死不救。");
      }
    }

    if (stage === "ripple") {
      if (slotId === "2.4") {
        lines.push("从这一步之后，你那层“只是顺手说两句”的轻松会越来越难维持。人群一旦开始意识到你不是全然无用，就会在关键时刻本能地回头找你。");
      }
      if (slotId === "3.3") {
        lines.push("尸体留下的回响不会老实待在原地。你之后也许只是扶一下门、递一句话，脑子里却会突然跳回这一次谁被你看得太清、谁被你按得太晚。");
      }
      if (slotId === "4.4") {
        lines.push("投票前后留下的回音尤其烦人，因为它会粘在每一次后续接触上。别人之后再看你时，很可能不会先想到你的玩笑，而会先想到你曾在哪个时刻把天平轻轻推向过谁。");
      }
      if (slotId === "5.1") {
        lines.push("异变之后，你那层壳已经不可能完整复原。就算之后还能继续演，演出来的也只会是一个大家已经知道“必要时能做什么”的版本。");
      }
      if (slotId === "5.4") {
